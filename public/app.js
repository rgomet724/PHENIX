...
async function logout() {
  try { await api('/api/logout', { method: 'POST' }); } catch { /* local logout anyway */ }
  state.token = '';
  state.user = null;
  sessionStorage.removeItem('pmPortalToken');
  showLogin();
...
async function saveApp(event) {
  ...
  try {
    await api('/api/admin/apps', { method: 'POST', body: JSON.stringify(payload) });
    resetAppForm();
    await loadAdmin();
  } catch (err) {
    error.textContent = err.message;
...
async function deleteApp(id) {
  ...
  await api(`/api/admin/apps/${encodeURIComponent(id)}`, { method: 'DELETE' });
  await loadAdmin();
}
...
async function resetUserPassword(id) {
  ...
  await api(`/api/admin/users/${encodeURIComponent(id)}/password`, {
    method: 'POST',
