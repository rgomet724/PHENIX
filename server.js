...
app.get('/api/data', needLogin, (req,res)=>{
  ...
  res.json({user:safe(u), agents:d.agents, callsigns:d.callsigns, interventions:d.interventions, crews:d.crews, logs:d.logs.slice(0,100), note:d.notes[u.id]||'', consignes:consignesForUser(d,u), users:u.role==='admin'?d.users.map(safe):undefined});
});
...
app.post('/api/consignes/:id/read', needLogin, (req,res)=>{
  const d=load(); const c=d.consignes.find(x=>x.id===req.params.id);
  if(!c) return res.status(404).json({error:'Consigne introuvable'});
  c.reads=c.reads||{}; c.reads[req.session.userId]=new Date().toISOString();
  save(d); res.json({ok:true});
});
...
app.post('/api/admin/users', needLogin, needAdmin, (req,res)=>{
  ...
  if(r.id){
    ...
    u.displayName=r.displayName; u.login=r.login; u.role=r.role; u.brigades=brigades;
  ...
  audit(d,req,'Gestion utilisateur'); save(d); res.json({ok:true});
});
...
app.listen(PORT,()=>console.log('PEGASE V28 consignes prêt sur le port '+PORT));
